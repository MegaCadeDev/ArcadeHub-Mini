...
// Controlller Connected Check Switch
typedef enum {
    CTL_UNKNOWN = 0,
    CTL_WIIU_PRO,
	CTL_WII,
...
static void fill_gamepad_report(int idx, uni_gamepad_t *gp, controller_type_t type)
{
	...
	// shoulder buttons
	if (type == CTL_WII){
		if ((gp->buttons & WII_MASK_L)) {
			report[idx].buttons |= SWITCH_MASK_L;
		}
		if ((gp->buttons & WII_MASK_R)) {
	...
	// triggers
	// Check if specific controller	
	if (type == CTL_WIIU_PRO || type == CTL_WII) {
		if (gp->buttons & WIIU_MASK_ZR)
			report[idx].buttons |= SWITCH_MASK_ZR;
		if (gp->buttons & WIIU_MASK_ZL)
...
static uni_error_t pico_switch_platform_on_device_ready(uni_hid_device_t* d) {
	...
	else if (d->vendor_id == WII_VID && d->product_id == WII_PID){
		type = CTL_WII;
	}

    // Store controller type in platform_data[0]
...
static void pico_switch_platform_on_controller_data(uni_hid_device_t* d, uni_controller_t* ctl)
{
	...
	// START + SELECT Button combo detection
	...
	if (combo_pressed && !disconnect_combo_active[idx]) {
		...
		logi("Disconnect combo pressed on the controller %d\n", idx);

		// Disconnect the controller
		uni_hid_device_disconnect(d);

		// Start scanning for a new controller
		uni_bt_enable_new_connections_safe(true);
	...
	if (!combo_pressed) {
		disconnect_combo_active[idx] = false;
	}

	controller_type_t type = (controller_type_t)d->platform_data[0];
	fill_gamepad_report(idx, gp, type);
	idx_r.idx = idx;
